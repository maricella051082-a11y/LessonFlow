...
function renderTeacherPremiumHome() {
    ...
    const pending = cloudMode ? cloudSubmissions.filter(function(item) { return item.status === "submitted"; }) : []; const returned = cloudMode ? cloudSubmissions.filter(function(item) { return item.status === "returned"; }) : []; const preparation = events.filter(function(item) { return item.status === "scheduled"; }); const attention = document.createElement("article"); attention.className = "teacher-attention-card"; addTextElement(attention, "p", "teacher-eyebrow", "ТРЕБУЕТ ВНИМАНИЯ"); addTextElement(attention, "h2", "", "На сегодня"); const attentionItems = []; if (pending.length) attentionItems.push(["assets/teacher/attention/teacher-icon-review.png", pending.length + " работ на проверке", function() { document.querySelector(".teacher-home-work-grid")?.scrollIntoView({ behavior: "smooth" }); }]); if (returned.length) attentionItems.push(["assets/teacher/attention/teacher-icon-returned.png", returned.length + " возвращённых заданий", function() { openTeacherDestination("students"); }]); if (preparation.length) attentionItems.push(["assets/teacher/attention/teacher-icon-preparation.png", preparation.length + " уроков требуют подготовки", function() { openTeacherDestination("preparation"); }]); if (attentionItems.length) attentionItems.forEach(function(item) { const row = document.createElement("button"); row.type = "button"; row.className = "teacher-attention-row"; const image = document.createElement("img"); image.src = item[0]; image.alt = ""; row.appendChild(image); addTextElement(row, "span", "", item[1]); addTextElement(row, "b", "", "→"); row.addEventListener("click", item[2]); attention.appendChild(row); }); else { const clear = document.createElement("div"); clear.className = "teacher-all-clear"; const image = document.createElement("img"); image.src = "assets/teacher/dashboard/teacher-all-clear.png"; image.alt = ""; clear.appendChild(image); const text = document.createElement("div"); addTextElement(text, "strong", "", "Всё в порядке"); addTextElement(text, "p", "", "Сейчас ничего не требует внимания."); clear.appendChild(text); attention.appendChild(clear); }
